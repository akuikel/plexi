...
export default function VoiceDemo() {
  ...
  useEffect(() => { stopEverything(); setCurrentLineIndex(-1); }, [activeTab]);
  ...
  const speakLine = (index: number) => {
    ...
    const v = getVoice(line.speaker);
    if (v) u.voice = v;
    u.rate = 0.95;
    u.pitch = line.speaker === 'AI' ? 1.05 : 0.95;
    u.onend = () => {
      if (!isPlayingRef.current) return;
      timeoutRef.current = setTimeout(() => speakLine(index + 1), 800);
    };
    window.speechSynthesis.speak(u);
  };

  const togglePlay = () => {
    if (isPlaying) { stopEverything(); return; }
    setIsPlaying(true);
  ...
  return (
    <div style={{
      backgroundColor: '#FAF8F5', borderRadius: '20px',
      ...
      {/* Tabs */}
      <div style={{
        display: 'flex', flexWrap: 'wrap', overflowX: 'auto', scrollbarWidth: 'none',
        justifyContent: 'space-between', gap: '5px', padding: '15px',
        borderBottom: '1px solid rgba(0,0,0,0.05)', backgroundColor: '#fff'
      }}>
        {Object.entries(DEMO_DATA).map(([key, data]) => (
          <button key={key} onClick={() => setActiveTab(key)} style={{
            ...
            flex: '1 0 auto', minWidth: 'fit-content', textAlign: 'center',
      ...
      <div style={{ padding: '20px', backgroundColor: '#fff' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '15px' }}>
          <button onClick={togglePlay} style={{
            ...
            backgroundColor: currentScenario.color, border: 'none',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            color: '#fff', fontSize: '1.2rem', cursor: 'pointer', flexShrink: 0
          }}>{isPlaying ? '■' : '▶'}</button>
          <div style={{ flex: 1 }}>
            <div style={{ height: '6px', backgroundColor: '#eee', borderRadius: '3px', overflow: 'hidden', marginBottom: '5px' }}>
              <div style={{ width: `${progressPercent}%`, height: '100%', backgroundColor: currentScenario.color, transition: 'width 0.5s ease' }} />
            </div>
          ...
          <button onClick={stopEverything} style={{ border: 'none', background: 'transparent', color: '#aaa', fontSize: '1.2rem', cursor: 'pointer' }}>⟳</button>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', backgroundColor: currentScenario.color, color: '#fff', padding: '6px 12px', borderRadius: '8px', fontSize: '0.8rem', fontWeight: '600', fontFamily: 'Inter, sans-serif' }}>
            <span>✨</span> AI Agent
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#555', fontSize: '0.85rem', fontWeight: '500', padding: '6px 12px', backgroundColor: '#f5f5f5', borderRadius: '8px', fontFamily: 'Inter, sans-serif' }}>
            📞 Caller
          </div>
        </div>
      </div>
      ...
      <div ref={transcriptionRef} style={{
        height: '350px', overflowY: isPlaying ? 'auto' : 'hidden',
        padding: '20px', backgroundColor: '#FAF8F5',
        display: 'flex', flexDirection: 'column', gap: '15px',
        scrollBehavior: 'smooth', position: 'relative',
        backgroundImage: `url(${currentScenario.image})`,
        backgroundSize: 'cover', backgroundPosition: 'center',
        transition: 'all 0.5s ease', cursor: !isPlaying ? 'pointer' : 'default'
      }} onClick={!isPlaying ? togglePlay : undefined}>
        <div style={{
          position: 'absolute', top: 0, left: 0, right: 0, bottom: 0,
          backgroundColor: isPlaying ? 'rgba(255,255,255,0.90)' : 'rgba(0,0,0,0.2)',
          backdropFilter: isPlaying ? 'blur(5px)' : 'none', zIndex: 0, transition: 'all 0.5s ease'
        }} />
        <div style={{ position: 'relative', zIndex: 1, height: '100%' }}>
          {!isPlaying && (
            <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
              <div style={{ width: 0, height: 0, borderTop: '30px solid transparent', borderBottom: '30px solid transparent', borderLeft: '50px solid rgba(255,255,255,0.95)', marginLeft: '8px', filter: 'drop-shadow(0 4px 12px rgba(0,0,0,0.4))', cursor: 'pointer' }} />
            </div>
          )}
          {isPlaying && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', paddingBottom: '20px' }}>
              ...
              {currentScenario.transcript.map((line, index) => {
                ...
                return (
                  <div key={index} style={{ alignSelf: line.speaker === 'AI' ? 'flex-start' : 'flex-end', maxWidth: '85%', animation: 'slideUp 0.4s ease-out' }}>
                    <div style={{
                      ...
                      fontSize: '0.9rem', lineHeight: '1.5', fontFamily: 'Inter, sans-serif',
